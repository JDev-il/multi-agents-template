 ...
 *
 * Creates a new Git Worktree, generates .claude-scope and TASK.md,
 * and opens your IDE at the correct path.
 */
...
// ── .claude-scope generator ───────────────────────────────────────────────────

const generateClaudeScope = ({ project, agent, branchName }) => {
  return `# .claude-scope
...
# If scope does not match the loaded CLAUDE.md — hard stop.
...
When Claude Code loads this file it must verify:
1. The loaded CLAUDE.md matches the project scope above
2. If opened at repo root instead of this worktree — hard stop with:

   WRONG CONTEXT — CANNOT PROCEED
   This worktree is scoped to: ${project}/${agent}
   You have opened Claude Code at the wrong path.
   Close and reopen at: worktrees/${branchName.replace('agent/', '').replace('/', '-')}

3. Reference the correct agent file: agents/${agent}.md
4. Read TASK.md for the current task definition and prompt
`;
};
...
const main = async () => {
  ...
  try {
    execSync(`git worktree add worktrees/${worktreeName} -b ${branchName}`, {
      cwd: ROOT,
      stdio: 'pipe',
    });
  ...
  fs.writeFileSync(
    ...
    generateClaudeScope({ project, agent, branchName }),
    'utf8'
  );
  ...
  fs.writeFileSync(
    ...
    generateTaskMd({ project, agent, task, branchName, worktreePath }),
    'utf8'
  );
  ...
  const ideCmd = config.ide?.cmd;
  if (ideCmd) {
    try {
      execSync(`${ideCmd} ${worktreePath}`, { stdio: 'pipe' });
      console.log(`  ${green('✓')} ${config.ide.label} opened at worktrees/${worktreeName}`);
    } catch (err) {
      console.log(`  ${yellow('!')} Could not open ${config.ide.label} automatically.`);
      console.log(dim(`     Open manually at: ${worktreePath}`));
    }
  } else {
    console.log(`  ${yellow('!')} Open your IDE manually at:`);
  ...
  console.log(`  ${bold('2.')} Open ${cyan('TASK.md')} — it contains your exact Claude Code prompt.\n`);
  console.log(`  ${bold('3.')} Open a ${bold('NEW')} Claude Code chat window.`);
  console.log(dim(`     Do NOT reuse a previous session.\n`));
  console.log(`  ${bold('4.')} Copy the prompt from TASK.md and paste it into the chat.\n`);
  console.log(`  ${bold('5.')} When the agent completes the task:`);
